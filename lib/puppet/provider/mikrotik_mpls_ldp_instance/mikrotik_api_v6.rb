...
Puppet::Type.type(:mikrotik_mpls_ldp_instance).provide(:mikrotik_api_v6, :parent => Puppet::Provider::Mikrotik_Api) do
  ...
  def self.mplsLdp(data)
    ...
    new(
      ...
      :state                        => state,
      :lsr_id                       => data['lsr-id'],
      :transport_addresses          => [data['transport-address']],
  ...
  def flush
    ...
    update["transport-address"] = resource[:transport_addresses].first if ! resource[:transport_addresses].nil?
