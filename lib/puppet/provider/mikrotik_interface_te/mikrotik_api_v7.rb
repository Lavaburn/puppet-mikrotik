...
Puppet::Type.type(:mikrotik_interface_te).provide(:mikrotik_api_v7, :parent => Puppet::Provider::Mikrotik_Api) do
  ...
  def self.instances
    interfaces = Puppet::Provider::Mikrotik_Api::get_all("/mpls/traffic-eng/tunnel")
  ...
  def self.teInterface(data)    
    ...
    Puppet.info("Hops: #{data.inspect}")    # TODO rm
    
    record_route = :false
    if !data['record-route'].nil? && data['record-route'] == 'yes'
      record_route = :true      
    end
  ...
  def flush 
    Puppet.debug("Flushing MPLS TE Tunnel #{resource[:name]}")
    ...
    Puppet.info("Params: #{params.inspect} - Lookup: #{lookup.inspect}")    # TODO debug

    simple_flush("/mpls/traffic-eng/tunnel", params, lookup)
  end
end