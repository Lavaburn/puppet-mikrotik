...
Puppet::Type.type(:mikrotik_ip_route).provide(:mikrotik_api_v7, :parent => Puppet::Provider::Mikrotik_Api) do
  ...
  def self.ipRoute(data)
    if data['comment'] != nil
      ...
      new(
        ...
        # TODO: supress HW offload
        :type                 => type,
        :distance             => data['distance'],
        :scope                => data['scope'],
        :target_scope         => data['target-scope'],
        :routing_mark         => data['routing-table'],
  ...
  def flush
    ...
    if !resource[:type].nil?
      if resource[:type] == "unicast"
        params["blackhole"] = false 
      else 
    ...
    params["routing-table"] = resource[:routing_mark] if !resource[:routing_mark].nil?
