...
Puppet::Type.type(:mikrotik_ip_route_rule).provide(:mikrotik_api_v7, :parent => Puppet::Provider::Mikrotik_Api) do
  ...
  def self.instances   
    ...
    rules = Puppet::Provider::Mikrotik_Api::get_all("/routing/rule")
    rules.each do |rule|
      object = ipRule(rule)
      if object != nil        
        instances << object
  ...
  def flush
    Puppet.debug("Flushing Routing Rule #{resource[:name]}")
    ...
    simple_flush("/routing/rule", params, lookup)
  end  
end
