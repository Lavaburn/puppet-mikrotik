...
Puppet::Type.type(:mikrotik_tool_email).provide(:mikrotik_api_v7_12, :parent => Puppet::Provider::Mikrotik_Api) do
  confine :feature => :mtik
  confine :feature => :ros_v7_12
  ...
  def self.toolEmail(data)
    new(
      ...
      :enable_starttls => convertYesNoToBool(data['tls']).to_s
      # TODO: VRF
    )
  end
  ...
  def flush
    ...
    update = {}
    update["server"] = resource[:server] if ! resource[:server].nil?
    ...
    update["tls"] = Puppet::Provider::Mikrotik_Api::convertBoolToYesNo(resource[:enable_starttls]) if ! resource[:enable_starttls].nil?
