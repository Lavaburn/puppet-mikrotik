...
describe 'IPv4 DHCP' do
  ...
  describe '/ip/dhcp' do  
    ...
    context "v6 - enable dhcp server" do
      it 'should update master' do
        site_pp = <<-EOS
          mikrotik_dhcp_server { 'DHCPD1':
            ensure => enabled
      ...
      # ROSv6: Server will be enabled
      # ROSv7: Nothing will change as server is enabled by default
      it_behaves_like 'an idempotent device run after failures', 0
    end
